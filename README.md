...
Требования: Docker Engine 24+ и Docker Compose v2 (поддерживается также standalone `docker-compose`). Для локальной Qwen рекомендуется 16 ГБ RAM и 20 ГБ свободного места.
...
При первом запуске создаётся `.env`: `DB_PASSWORD` и `JWT_SECRET` генерируются автоматически, а на Unix устанавливаются права `600`. Скрипт скачивает утверждённый `Qwen3-1.7B-Q4_K_M.gguf` и обязательно сверяет его SHA-256. Запуск останавливается, если checksum отсутствует, имеет неверный формат или не совпадает.
...
- Выбор только реально доступного провайдера: локальная Qwen, DeepSeek или GigaChat.
...
Пять контейнеров: `frontend`, `api-core`, `postgres`, `ai-driver`, `qwen-local`. Снаружи публикуются frontend, а диагностические API привязаны к `127.0.0.1`; PostgreSQL и Qwen доступны только во внутренней Docker-сети. Очередь и результаты хранятся в PostgreSQL, Data Protection keys — в отдельном volume.

Локальный production-профиль:
...
Разработано для финала хакатона Корпоративного университета Санкт-Петербурга. © 2025–2026.
