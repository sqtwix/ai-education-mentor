...
FRONTEND_PORT=80
API_HOST_PORT=5050
AI_DRIVER_HOST_PORT=8000
```

Все три порта должны быть разными числами от 1 до 65535. Frontend публикуется на всех интерфейсах, API Core и AI Driver — только на loopback.

### Режим без локальной LLM

Это штатный режим по умолчанию и не требует добавлять переменную вручную. Если `ENABLE_LOCAL_LLM` отсутствует, deploy трактует её как `false`. Запускаются `frontend`, `api-core`, `postgres` и `ai-driver`; managed-контейнер останавливается, но GGUF не удаляется. Параметр не отключает DeepSeek или GigaChat: их доступность определяется собственными ключами. Если все ключи также пусты, доступны регистрация, вход, каталог, аналитика, история, архив и настройки, а создание новой ИОТ возвращает `503 MODEL_UNAVAILABLE` без постановки задачи в очередь.
...
Поддерживаются модели `.gguf`, совместимые с закреплённым llama.cpp и chat completions. Имя файла не может содержать путь; SHA-256 обязателен. Новый файл скачивается только по HTTPS во временный `.part`, проверяется и затем атомарно переименовывается. Автозагрузка выполняется только в managed-режиме и только если `models/LOCAL_LLM_MODEL_FILE` отсутствует. Для air-gapped установки заранее скопируйте файл в `models/`: URL тогда не используется, но checksum всё равно проверяется.
...
Безопасное переключение не требует удаления контейнеров или volumes:

1. измените ровно одно значение каждого нужного ключа в `.env`;
2. для managed-файла заранее получите утверждённый SHA-256;
3. выполните `./deploy.sh` или `deploy.bat`;
4. проверьте `/models/availability` и создайте одну контрольную ИОТ;
5. если выбран `false` или `external`, оставшийся managed-контейнер будет удалён, но GGUF в `models/` сохранится.
...
Они необязательны. Пустой ключ делает соответствующую модель недоступной в интерфейсе. Наличие ключа означает «настроен», а не проверку баланса или лимита; после настройки выполните один тестовый запрос. Перед включением внешнего провайдера согласуйте передачу обезличенных данных, договор, лимиты, журналирование и требования организации.
...
Получение и развёртывание обновления:
...
Deploy сохраняет существующие значения `.env`, дополняет отсутствующие ключи из шаблона, не удаляет PostgreSQL/Data Protection volumes и не удаляет GGUF. EF Core migrations применяются API при старте. Не откатывайте приложение на схему, несовместимую со старой версией; при необходимости восстанавливайте согласованную пару приложения и backup БД.
...
### Deploy остановился до сборки

Deploy намеренно останавливается до изменения контейнеров, если `.env` содержит дубликаты ключей, некорректный boolean, одинаковые/недопустимые порты, недоступный Docker Engine или невалидный Compose. Исправьте конкретную ошибку и повторите ту же команду: повторный запуск идемпотентен. Не добавляйте вторую строку с тем же ключом: оставьте одно значение.
