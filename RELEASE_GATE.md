...
Проект считается готовым к production-демо только после выполнения всех пунктов. Состояние ниже подтверждено прогоном 23 августа 2026 года; незакрытые пункты не маскируются кодовыми значениями по умолчанию.
